...
/** Path to the A3_LAUNCHPAD_EXT data directory (same as live map) */
static std::string getDataDir() {
	return getLiveMapDir();
}

/** Ensure the A3_LAUNCHPAD_EXT data directory exists. */
static bool ensureDataDirExists(std::string& outDir) {
	outDir = getDataDir();
	try {
		...
		Logging::logError(("Failed to create A3_LAUNCHPAD_EXT data directory: " + std::string(e.what())).c_str());
		return false;
	}
}
...
static bool writeJsonToFile(const std::string& filePath, const json& j) {
	std::lock_guard<std::mutex> lock(g_liveMapFileMutex);
	try {
