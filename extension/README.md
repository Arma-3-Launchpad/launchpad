...
cmake --build out/build --config Release   # add --config Release on Visual Studio generators
```

Artifacts are copied next to the repo mod layout at `../bin/mod/` (POST_BUILD in `CMakeLists.txt`).
